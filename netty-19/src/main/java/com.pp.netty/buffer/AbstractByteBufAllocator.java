...
public abstract class AbstractByteBufAllocator implements ByteBufAllocator {
    ...
    //所以要怎么做？把ByteBuf交给弱引用对象，如果ByteBuf被垃圾回收了，但是弱引用对象中的ByteBuf引用并没有被清除，那该ByteBuf就会被放到
    //一个弱引用队列中，Netty就可以检查该弱引用队列来判断是否出现内存泄漏了。只有被包装过ByteBuf执行了release方法，才会讲自身从对应的
    //弱引用对象中清除，这样被垃圾回收后也不会被放大弱引用队列中。而在执行release方法的过程中，还会释放自身持有的内存。由此做到了内存泄漏的检测
