...
import com.pp.netty.util.internal.InternalThreadLocalMap;
...
//        }
    ...
    public void removeBytes(long writtenBytes) {
        for (;;) {
            ...
            //在这里计算出ByteBuf中剩余的可读字节数，也就是可以发送的字节数
            ...
            if (readableBytes <= writtenBytes) {
                ...
                if (writtenBytes != 0) {
                    ...
                    //大家可以举个例子代入一下，比如一共有三个ByteBuf，每个存放两个字节，一共有两个字节，但是刷新了5个字节
    ...
    void failFlushed(Throwable cause, boolean notify) {
        //如果正在执行失败处理(inFail为true)，则直接返回，不做任何处理
        if (inFail) {
            return;
        }
        try {
            //设置正在进行失败处理的标志为true
            inFail = true;
            //无限循环，直到remove0方法返回false为止
            for (;;) {
                //这里就会删除Entry链表中的节点，直到删除完
                //注意，这里删除的时候并不是说会直接删除Entry连标中的每一个节点
                //会先对flushedEntry进行判断，如果flushedEntry为null，说明此时没有数据要刷新到Socket输出缓冲区中了
                //因为只有当Channel调用了flush方法后，flushedEntry才会被赋值
                //如果flushedEntry不为null，则把要发送的数据清空，因为Channel已经关闭了
    ...
    void close(final Throwable cause, final boolean allowChannelOpen) {
        //如果已经在执行失败处理(inFail为true)，将关闭操作提交到单线程执行器进行异步处理
        if (inFail) {
        ...
        //设置正在进行失败处理的标志为true
        inFail = true;
        //如果不允许在通道打开时执行关闭操作，且通道当前为打开状态，则抛出异常
        ...
        try {
            ...
            while (e != null) {
                //获取当前条目的待处理数据大小
                int size = e.pendingSize;
                //从总的待处理数据大小中减去当前条目的数据大小
                TOTAL_PENDING_SIZE_UPDATER.addAndGet(this, -size);
                //如果当前条目未被取消，则释放条目中的消息，并将条目对应的promise设置为失败状态
                if (!e.cancelled) {
