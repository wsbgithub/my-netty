...
import com.pp.netty.channel.socket.ServerSocketChannel;
import com.pp.netty.channel.socket.SocketChannelConfig;
import com.pp.netty.util.concurrent.GlobalEventExecutor;
...
import static com.pp.netty.channel.internal.ChannelUtils.MAX_BYTES_PER_GATHERING_WRITE_ATTEMPTED_LOW_THRESHOLD;

/**
 * @Author: PP-jessica
 * @Description:对socketchannel做了一层包装，同时也因为channel接口和抽象类的引入，终于可以使NioEventLoop和channel解耦了
 */
public class NioSocketChannel extends AbstractNioByteChannel implements com.pp.netty.channel.socket.SocketChannel{
    ...
    public boolean isShutdown() {
        ...
        //这里会判断读关闭和写关闭，判断channel是不是活跃的
        return socket.isInputShutdown() && socket.isOutputShutdown() || !isActive();
    }

    /**
     * @Author: PP-jessica
     * @Description:下面这两个方法没用到，因此就空实现了
     */
    @Override
    ...
    /**
     * @Author: PP-jessica
     * @Description:这个就是channel半关闭方法的入口
     */
    @Override
    ...
    /**
     * @Author: PP-jessica
     * @Description:在这个方法中会继续将半关闭方法向下调用
     */
    @Override
    ...
    private final class NioSocketChannelUnsafe extends NioByteUnsafe {
        /**
         * @Author: PP-jessica
         * @Description:说实话，我整理代码到这里的时候，也是第一次遇见这个东西
         * 之前我也没用过这个TCP配置参数，这里也是我现查的，大家可以简单看看，如果有自己的见解
         * 可以发到群里交流交流
         */
        @Override
        protected Executor prepareToClose() {
            try {
                //看见config()其实就能才到，这一定和配置参数有关。这里得到的其实就是底层SocketOptions.SO_LINGER配置的值
                //并且如果配置的这个值大于0，那么当我们的程序执行了close方法，程序就会阻塞在该方法处，直到TCP缓冲区中的数据全部发送
                //完毕，并且收到对方的确认消息后，才会结束close的阻塞，程序继续执行下去。
                //如果用户设定的这个参数的值是3s，假如超过了设定的这个时间，刷新缓冲区的动作还未完成，这时候，不管怎样都会结束close的阻塞
                //这就是这个配置参数的作用
                //其实设置这个配置参数就相当于暂时不关闭channel，等活干完了再进行关闭channel的动作
                //如果没有配置这个参数的值，或者说这个值等于0，那调用close方法就不会阻塞，会立即返回
                if (javaChannel().isOpen() && config().getSoLinger() > 0) {
                    //这里就是用户设置了延时关闭channel的参数的值，但下面这个方法是什么意思呢？是取消
                    //channel在多路复用器上注册的事件。为什么这里要执行这样一个操作呢？弄清楚这些还是很重要的，至少能让你
                    //明白Netty的设计有多精细，为性能考虑到了极致。同时，这里其实也催生出了一个bug
                    //在执行close的情况下，这时候没有半关闭，而是读写都关闭，这时候channel是不能再读取和发送消息的了
                    //但是走到这里就意味着用户设置了这个参数的值，是大于0的，延时这么多时间再执行close的动作
                    //在这期间又可以读取和发送消息了，但是都要关闭channel了，并且调用的是close方法，难道还会在这个时间
                    //向服务端发送消息吗？应该不会，所以这里就调用了下面的这个方法，把要关闭的channel注册在多路复用器上的
                    //事件取消，这样selector在轮训的时候，就不必关注这个要关闭的channel了，间接提高了一点点性能
                    //但同时bug也随之而来，假如是在半关闭状态下，客户端还可以接收消息，但这时候你把读写事件全取消了
                    //不就很尴尬了吗。。所以，这个地方在后面的版本中被修复了
                    //感兴趣的朋友可以继续挖掘一下
                    doDeregister();
                    //这里是个重点，可以看到，这里返回了一个全局执行器，就是一个单独的执行器，这个执行器在接下来会取代单线程执行器
                    //来执行关闭channel的方法
                    //为什么要搞一个新的执行器呢？原因很简单，程序在这里走的是配置参数大于0的分支，也就是说在执行到close方法时，会阻塞一会
                    //如果是单线程执行器来执行，那单线程阻塞住了，它管理的其他没有关闭的channel怎么收发消息呢？用户提交的异步任务
                    //又该怎么执行呢？所以这里就搞了一个新的执行器，暂时去替代单线程执行器工作，执行close方法。这样，即便阻塞
                    //阻塞的也只是这个新的执行器
                    return GlobalEventExecutor.INSTANCE;
                }
            } catch (Throwable ignore) {
            }
            //如果走到这里，说明用户没有设置配置参数的值，那执行close方法就不会阻塞，返回null就行，仍由单线程执行器去执行close方法
            return null;
        }
    }
