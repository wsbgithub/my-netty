...
import static com.pp.netty.util.internal.MathUtil.safeFindNextPositivePowerOfTwo;
...
public abstract class Recycler<T> {
    ...
    /**
     * @Author: PP-jessica
     * @Description:这是一个辅助属性，帮助计算每个线程可以帮助其他线程回收的最大对象个数
    ...
    //MAX_SHARED_CAPACITY_FACTOR这个辅助属性会赋值给该属性，帮助计算每个线程可帮助其他线程回收的最多对象个数
    ...
    protected Recycler(int maxCapacityPerThread, int maxSharedCapacityFactor,
        ...
        if (maxCapacityPerThread <= 0) {
            ...
            //帮助计算每个线程可帮助其他线程回收的最多对象个数的辅助属性赋值
    ...
    private static final class WeakOrderQueue {
        ...
        static final class Link extends AtomicInteger {
            ...
            //这个相当于读指针，实际上就是容器中可以读取的对象的个数，代表该link节点中存储了多少个待被回收的对象
         ...
         * 看看该WeakOrderQueue对象对应的线程是否帮助其他线程回收的对象达到了上限
         *
         */
        ...
        boolean transfer(Stack<?> dst) {
            ...
            if (srcStart != srcEnd) {
                ...
                //这里是判断，经过转移后，创建线程的栈容量有没有变化，是不是还是旧的容量，也就是存储的元素个数没变
    ...
    static final class Stack<T> {
        ...
        private void pushNow(DefaultHandle<?> item) {
            //如果是线程回收自己对象池中的对象，那么handle中的这两个属性这是偶应该都还是0，是相等的。
            //其中有一个不是0，就说明已经被回收了，或者至少被放在WeakOrderQueue中了
            if ((item.recycleId | item.lastRecycledId) != 0) {
                throw new IllegalStateException("recycled already");
            }
            //自己回收自己的线程的时候，handle中的这两个属性都会被赋值为OWN_THREAD_ID
        ...
        private void pushLater(DefaultHandle<?> item, Thread thread) {
            ...
            //注意啊，这里的这个this，就是帮助原stack所属的那个线程的stack，因为pushLater这个方法是在stack
            //中被调用的，可以自己去看看调用链，这个方法是由 stack.push(this)方法一路调用至此的
            //这里就是想得到WeakHashMap中stack对应的value
            WeakOrderQueue queue = delayedRecycled.get(this);
            if (queue == null) {
                //如果为空，就说明帮助其他线程回收对象的本线程是第一次帮助其他对象回收线程，所以要创建一个WeakOrderQueue对象
        ...
        /**
         * @Author: PP-jessica
         * @Description:该方法只有返回false，才能使对象被真正回收
         */
