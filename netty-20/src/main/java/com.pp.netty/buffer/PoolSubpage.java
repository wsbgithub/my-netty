...
//该对象为链表的节点，该对象构成了PoolSubpage数组中每一个节点的链表
/**
 * @Author: PP-jessica
...
final class PoolSubpage<T> implements PoolSubpageMetric {
    ...
    //一个PoolSubpage位8KB，最多可以分配成512个16B，所以数组容量最大为
    ...
    //bitmap数组的容量大小
    //这里为什么要有这个属性呢？因为每一个PoolSubpage可能是按照不同的elemSize均分的，所以分成的内存块的数目也相同
    ...
    /**
     * @Author: PP-jessica
     * @Description:特殊的构造器方法，创建PoolArena中PoolSubpage数组中存储的双向链表的头节点的方法
     */
    ...
    void init(PoolSubpage<T> head, int elemSize) {
        ...
        if (elemSize != 0) {
            //根据elemSize计算出啊这个PoolSubpage可以被分成多少块
            //就是让8KB除以elemSize
            //这里把还剩下可以分配的内存块也赋值了
    ...
    /**
     * @Author: PP-jessica
     * @Description:分配内存的方法，返回的就是位图中的一个索引下标
    ...
    */
    ...
    /**
     * @Author: PP-jessica
     * @Description:把该PoolSubpage添加到PoolArena中PoolSubpage数组相应的双向链表中的方法
     * 添加到头节点之后，使用的是头插法
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:把该PoolSubpage从PoolArena中PoolSubpage数组相应的双向链表中删除的方法
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:真正寻找下一个可以分配的内存块在数组中的位置的方法
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:从bitmap位图数组中找到可以分配出去的内存的方法
    ...
    private int findNextAvail0(int i, long bits) {
        ...
        for (int j = 0; j < 64; j ++) {
            ...
            //得到的值肯定不是0，因为低四位都为1
            //所以不会走下面的分之，会直接走到 bits >>>= 1代码，把bits右移一位，然后再次和1做与运算，当右移了4次之后
    ...
    /**
     * @Author: PP-jessica
     * @Description:把这个算出来的在位图数组中的数组下标转换成一个具体的内存句柄
     */
