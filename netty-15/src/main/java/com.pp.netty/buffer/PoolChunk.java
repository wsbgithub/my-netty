...
final class PoolChunk<T> implements PoolChunkMetric {
    ...
    //从第一个叶子结点把内存地址返回出去，因为是第一个叶子结点，在memoryMap数组中的偏移量为0，也就意味着在Chunk这个内存块中的偏移量
    ...
    //要改变一点。如果这时候再来分配一个8KB的内存，就从11层找到第二个叶子节点，这个节点在数组中的偏移量为1，而1就代表8KB，所以
    ...
    //而这个PoolSubpage数组的容量就是2048。其实就是Chunk没分配出一个叶子结点大小的内存，就会使用一个PoolSubpage来包装一下
    ...
    //如果与运算后的值不为0了，则说明用户申请的内存大于8KB，就不能从PoolSubpage中分配了，因为PoolSubpage的最大值为8KB
    private final int subpageOverflowMask;
    //其实就是PoolSubpage的大小，为8KB，也就是8192
    private final int pageSize;
    //这个属性就是用来快速计算用户申请的内存经过规整之后要从二叉树的那一层开始分配
    ...
    /**
     * @Author: PP-jessica
     * @Description:非池化的构造器，不是我们的重点内容，所以就略过了
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:分配内存的方法，这里的参数buf，还记得我们上面说的吧，分配的内存会被包装在BytreBuf中，也就是PooledByteBuf
    ...
    boolean allocate(PooledByteBuf<T> buf, int reqCapacity, int normCapacity) {
        ...
        //数组起始地址的偏移量吗？
        //而该数值的高32位则代表了内存是从PoolSubpage数组中分配的，一个PoolSubpage位8KB，最多可以分配成512个16B，所以数组容量最大为
        ...
        if ((normCapacity & subpageOverflowMask) != 0) {
            //走到这里说明分配的是大于8KB的内存
            //既然是大于8KB的，请大家注意，那分配出来的一定就是叶子结点了
    ...
    /**
     * @Author: PP-jessica
     * @Description:递归更新父节点对应的二叉树数组存储的层数值
    ...
    /**
     * @Author: PP-jessica
     * @Description:递归更新父节点状态值的方法
     * 逻辑实在是写不动了，大家对照着updateParentsAlloc方法，自己品味品味这个方法的逻辑吧
     * 都不难，真的，代入几个具体的数，逻辑就显现出来了，然后从逻辑中总结规律即可
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:从Chunk内存块中具体分配一块大于8KB的内存的具体方法
     * 参数就是要分配的内存在二叉树的第几层的那个层数
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:该方法就是从Chunk内存块中分配一个大于8KB的内存
    ...
    /**
     * @Author: PP-jessica
     * @Description:分配小于8KB的内存的核心方法，是从PoolSubpage中分配的
     */
    ...
    void initBuf(PooledByteBuf<T> buf, ByteBuffer nioBuffer, long handle, int reqCapacity) {
        ...
        int bitmapIdx = bitmapIdx(handle);
        if (bitmapIdx == 0) {
            //这里是判断了没有使用位图，就意味着是从Chunk中分配的超过8KB大小的内存，所以直接初始化Buf即可
    ...
    /**
     * @Author: PP-jessica
     * @Description:得到该节点处的索引对应在数组中存储的值
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:内存分配出去了，要把节点设置成不可分配的状态
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:计算的就是在相应的层数，每一个节点代表多大的内存
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:计算从Chunk内存块中分配出去的内存在Chunk内存块的memory中的具体偏移量
    ...
    private int runOffset(int id) {
        ...
        //id加入就是2048 二进制就为            0000  0000 0000 0000 0000 1000 0000 0000
        //位运算得到的就是0
    ...
    /**
     * @Author: PP-jessica
     * @Description:计算出当前要分配的8KB内存块在subpages数组中的位置
     * maxSubpageAllocs的值为2048
     */
    ...
    /**
     * @Author: PP-jessica
     * @Description:得到long的高32位，右移32，就得到高32为了
     *  Integer.SIZE正好是32
     */
