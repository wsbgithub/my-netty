...
public abstract class DefaultMaxMessagesRecvByteBufAllocator implements MaxMessagesRecvByteBufAllocator {
    ...
    public abstract class MaxMessageHandle implements ExtendedHandle {
        ...
        //该成员变量用于读取客户端channel发送的消息，和服务端接收客户端连接无关。
         ...
         * 加上前面的respectMaybeMoreData这个属性，这个属性默认值为false，所以 (!respectMaybeMoreData || maybeMoreDataSupplier.get())
         * 的意思就成了如果客户端channel中还有数据等待被读取，是不是需要被重视，也就是要去被重视地去强迫读取？
         * respectMaybeMoreData这个单词的翻译就是"重视也许更多的数据"。。反正就是这么个意思，但这个属性默认值为false，也就是说不用认真对待
         * 所以，就不用去读取了
